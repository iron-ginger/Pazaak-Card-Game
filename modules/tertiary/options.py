...
def change_speed(options):
	while True:
		try:
			choice = int(input("How slow would you like to messages to appear? 0, 1, 2, or 3 (3 is slowest): "))
		...
		except ValueError: print("Invalid input. Choose again: ")
#
def rules():
	with open("reads/rules.txt", 'r') as rulebook: print("\n{}\n\n".format(rulebook.read()))
#
def debug(options):
	while True:
		try:
			choice = input("Would you like to turn debug mode 'on' or 'off': ")
...
def run_amount(p1, p2):
	if p1['type'] == 'computer' and p2['type'] == 'computer':
		while True:
			try:
				return int(input("How many times would you like to run the sim?"))
